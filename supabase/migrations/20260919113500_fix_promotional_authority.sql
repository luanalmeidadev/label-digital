-- Fix promotional authority in RPCs

CREATE OR REPLACE FUNCTION public.create_online_order_atomic(p_customer_id uuid, p_address_id uuid, p_order_type text, p_payment_method text, p_cash_change_for numeric, p_delivery_fee numeric, p_notes text, p_items jsonb, p_coupon_code text DEFAULT NULL::text, p_timezone text DEFAULT 'America/Sao_Paulo'::text)
 RETURNS TABLE(order_id uuid, order_number bigint, subtotal numeric, delivery_fee numeric, total numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
BEGIN
  ...
  LOOP
    -- Explictly reset promotional variables to prevent leakage
    v_promo_record := NULL;
    v_observed_event_id := NULL;
    v_observed_promotional_base_unit_price := NULL;
    v_is_promo_line := NULL;
  ...
  FOR item_json IN SELECT value FROM jsonb_array_elements(p_items)
  LOOP
    -- Explictly reset to prevent residual leakage
    v_promo_record := NULL;

    -- Re-resolve authoritatively for persistence
    SELECT * INTO v_promo_record FROM public.resolve_active_promotion(
    ...
    INSERT INTO public.order_items (
      ...
      v_promo_record.event_id,
      v_promo_record.event_name,
      v_promo_record.promotional_price
    )
...
CREATE OR REPLACE FUNCTION public.create_configured_cashier_sale(p_cash_session_id uuid, p_cashier_reference uuid, p_items jsonb, p_payments jsonb, p_customer_name text DEFAULT NULL::text, p_notes text DEFAULT NULL::text)

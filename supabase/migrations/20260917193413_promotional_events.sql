-- 20260917193413_promotional_events.sql

CREATE TABLE IF NOT EXISTS public.promotional_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name text NOT NULL,
  active boolean NOT NULL DEFAULT true,
  schedule_type text NOT NULL CHECK (schedule_type IN ('weekly', 'period')),
  ...
  CONSTRAINT name_trim_not_empty CHECK (trim(name) <> ''),
  CONSTRAINT period_check CHECK (
    (schedule_type = 'weekly') OR (starts_at IS NOT NULL AND ends_at IS NOT NULL AND starts_at < ends_at)
  ),
  CONSTRAINT weekly_check CHECK (
    (schedule_type = 'period') OR (weekdays IS NOT NULL AND array_length(weekdays, 1) > 0 AND start_time IS NOT NULL AND end_time IS NOT NULL AND start_time < end_time)
  )
);

CREATE OR REPLACE FUNCTION public.check_weekdays_validity(days integer[])
RETURNS boolean
...
ALTER TABLE public.promotional_events ADD CONSTRAINT weekdays_range_check CHECK (public.check_weekdays_validity(weekdays));

CREATE EXTENSION IF NOT EXISTS moddatetime WITH SCHEMA extensions;

CREATE TRIGGER handle_updated_at BEFORE UPDATE ON public.promotional_events
  FOR EACH ROW EXECUTE PROCEDURE extensions.moddatetime(updated_at);

CREATE TABLE IF NOT EXISTS public.promotional_event_products (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id uuid NOT NULL REFERENCES public.promotional_events(id) ON DELETE CASCADE,
  product_id uuid NOT NULL REFERENCES public.products(id) ON DELETE CASCADE,
  variant_id uuid NULL REFERENCES public.product_variants(id) ON DELETE CASCADE,
  promotional_price numeric(10,2) NULL CHECK (promotional_price >= 0),
  availability_mode text NOT NULL DEFAULT 'inherit' CHECK (availability_mode IN ('inherit', 'available_during_event', 'unavailable_during_event')),
  UNIQUE(event_id, product_id, variant_id)
);

ALTER TABLE public.order_items
  ADD COLUMN IF NOT EXISTS promotional_event_id uuid NULL REFERENCES public.promotional_events(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS promotional_event_name text NULL,
  ADD COLUMN IF NOT EXISTS promotional_base_unit_price numeric(10,2) NULL;

ALTER TABLE public.promotional_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.promotional_event_products ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admin full access events" ON public.promotional_events
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.admin_profiles WHERE admin_profiles.id = auth.uid()));

CREATE POLICY "Admin full access event_products" ON public.promotional_event_products
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.admin_profiles WHERE admin_profiles.id = auth.uid()));
...
GRANT ALL ON TABLE public.promotional_event_products TO authenticated;
GRANT ALL ON TABLE public.promotional_event_products TO service_role;

-- Central SQL Resolver
CREATE OR REPLACE FUNCTION public.resolve_active_promotion(
  p_product_id uuid,
  p_variant_id uuid,
  p_timezone text,
...
DECLARE
  ...
  v_count integer;
BEGIN
  v_local_time := p_at AT TIME ZONE p_timezone;
  v_local_date := v_local_time::date;
  v_time := v_local_time::time;
  v_weekday := EXTRACT(DOW FROM v_local_time)::integer;

  SELECT count(*) INTO v_count
  FROM public.promotional_event_products pep
  JOIN public.promotional_events pe ON pe.id = pep.event_id
  WHERE pe.active = true
    AND pep.product_id = p_product_id
    AND (pep.variant_id IS NULL OR pep.variant_id = p_variant_id)
    AND (
      (pe.schedule_type = 'period' AND p_at >= pe.starts_at AND p_at <= pe.ends_at)
      OR
      (pe.schedule_type = 'weekly' AND v_weekday = ANY(pe.weekdays) AND v_time >= pe.start_time AND v_time <= pe.end_time)
    );
  ...
  ELSIF v_count = 1 THEN
    ...
    SELECT
      ...
      pep.promotional_price,
      pep.availability_mode
    FROM public.promotional_event_products pep
    JOIN public.promotional_events pe ON pe.id = pep.event_id
    WHERE pe.active = true
      AND pep.product_id = p_product_id
  ...
  ELSE
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'PROMOTION_CONFLICT';
  END IF;
END;
$$;

DROP FUNCTION IF EXISTS public.create_online_order_atomic(uuid, uuid, text, text, numeric, numeric, text, jsonb, text);
...
BEGIN
  ...
  LOOP
    ...
    IF v_promo_record.event_id IS NOT NULL THEN
      ...
      v_effective_base_price := COALESCE(v_promo_record.promotional_price, v_effective_base_price);
  ...
  LOOP
    INSERT INTO public.order_items (
      ...
      v_observed_event_id,
      v_promo_record.event_name,
      v_observed_promotional_base_unit_price
    )
...
DROP FUNCTION IF EXISTS public.create_configured_cashier_sale(uuid, uuid, jsonb, jsonb, text, text);
